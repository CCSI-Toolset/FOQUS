...
def main(args_to_parse=None):
    ...
    logging.getLogger("foqus." + __name__)\
        .debug('Working directory set to ' + os.getcwd())
    ...
    try:
        ...
        logging.getLogger("foqus." + __name__)\
            .exception("Could not load file specified with --load: "\
            + args.load)
        sys.exit(10)
    ##
    ...
    if args.saveValues:
        ...
        if not args.load:
            logging.getLogger("foqus." + __name__)\
                .error("To save values you must load a flowsheet")
            sys.exit(11)
        else:
            try:
    ...
    if args.loadValues:
        if not args.load:
            logging.getLogger("foqus." + __name__)\
                .error("To load values you must load a flowsheet")
            sys.exit(12)
        else:
            try:
    ...
    if args.run == "opt":
        ...
        try:
            ...
            while not terminate:
                ...
                if not guid:
                    ...
                    logging.getLogger("foqus." + __name__)\
                        .info("Job {0} will be canceled".format(jid))
                    db.add_message("consumer={0}, canceling job {1}"\
                    ...
                    # Session file to run
                    sfile = os.path.join(workingDirectory,
                        "session.foqus")
                    # result session file to keep on record
                    rfile = os.path.join(workingDirectory,
                        "results_session.foqus")
                    # Input values files
                    vfile = os.path.join(workingDirectory,
                        "input_values.json")
                    # Output values file
                    ofile = os.path.join(workingDirectory,
                        "output.json")
                    ...
                    try:
                        ...
                        logging.getLogger("foqus." + __name__)\
                            .info("Moving job {0} to running state"\
                            .format(jid))
                    except:
                        logging.getLogger("foqus." + __name__).exception(
            ...
            if guid:
                try:
                    db.add_message(
                        "consumer={0}, stopping consumer keyboard interupt"\
                        .format(consumer_uuid), guid)
                except:
                    pass
            logging.getLogger("foqus." + __name__)\
                .info("FOQUS Consumer stopped due to SIGINT")
            try:
                gt.terminate()
            except:
                pass
            ...
            if guid:
                try:
                    db.add_message(
                        "consumer={0}, stopping consumer exception, {1}"\
                        .format(consumer_uuid, str(e)), guid)
                except:
                    pass
            logging.getLogger("foqus." + __name__)\
                .exception("FOQUS Consumer stopped due to exception")
            exit_code = 3
            try:
                gt.terminate()
            except:
        ...
        if terminate:
            if guid:
                try:
                    db.add_message(
                        "consumer={0}, stopping consumer terminate status"\
                        .format(consumer_uuid), guid)
                    db.job_change_status(guid, "error")
                except:
                    pass
