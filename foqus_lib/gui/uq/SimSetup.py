...
class SimSetup(_SimSetup, _SimSetupUI):
    ...
    def loadSampleFile(self):
        ...
        sampleFile,_ = QFileDialog.getOpenFileName(
            self, "Load Sample file", "", "Psuade Files (*.dat *.psuade *.filtered);;Psuade Simple File (*.smp);;CSV (Comma delimited) (*.csv);;All files (%s)" % allFiles)
        ...
        try:
            ...
            traceback.print_exc()
            try:
                inputData, _, numInputs, _ = LocalExecutionModule.readDataFromSimpleFile(sampleFile)
        ...
        if fromSimpleFile and numLoadVariableInputs != numModelVariableInputs:
                prompt = "This file does not have the same number of variable inputs (%d) as the setup model (%d)!" % (numLoadVariableInputs, numModelVariableInputs)
                compatible = False
        elif not fromSimpleFile: # Check if input names match
            if numLoadVariableInputs < numModelVariableInputs:
                prompt = "This file does not have as many variable inputs (%d) as the setup model (%d)!" % (numLoadVariableInputs, numModelVariableInputs)
                compatible = False
            else:
        ...
        if not compatible:
            QMessageBox.warning(self, self.tr("UQ GUI"),
                               self.tr(prompt),
        ...
        if fromSimpleFile:
            ...
            for modelIndex, (modelInName, modelInType) in enumerate(zip(modelInputNames, modelInputTypes)):
                ...
                if modelInType == Model.FIXED:
                    ...
                    if modelInName not in loadInputNames:
                        ...
                        resultData[:, modelIndex] = [loadDefaults[loadIndex]] * numSamples
                    else:
                        inDataCol = inputData[:, loadIndex]
                        resultData[:, modelIndex] = inDataCol
