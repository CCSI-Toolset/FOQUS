...
class Visualizer:
    ...
    def xScatter(fname, x, cmd, star=None):
        ...
        if cmd == 'iplot1':
            ...
            if len(x) == 2:
                ...
                x2 = x[1]
                if x1 == x2:
                    error = 'Visualizer: In function xScatter(), %s requires unique elements in x.' % cmd.upper()
                    Common.showError(error)
                    return None
            else:
                error = 'Visualizer: In function xScatter(), %s expects x to be a list of length 2.' % cmd.upper()
                Common.showError(error)
                return None
    ...
    def showRSPlot(data, y, x, rsdim, ngrid, rsMethodName, vmin, vmax, mfile):
        ...
        if rsdim == 1:
            ...
            ptitle = 'Line Plot of "%s = %s(%s)"' % (ylabel, rsMethodName.upper(), xlabel)
            ...
            ylabel = x[1]
            zlabel = outVarName
            ftitle = '"2-Input to 1-Output" Visualization of %s Response Surface' % rsMethodName.upper()
            ptitle = 'Surface/Contour Plots of "%s = %s(%s, %s)"' % (zlabel, rsMethodName.upper(), xlabel, ylabel)
            Plotter.plotisoline(xmesh,ymesh,zdatm,ftitle,ptitle,xlabel,ylabel,zlabel)
        elif rsdim == 3:
            xdat = []
            ydat = []
            zdat = []
