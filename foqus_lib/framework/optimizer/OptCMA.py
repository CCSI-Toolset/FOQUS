...
class opt(optimization):
    ...
    def optimize(self):
        ...
        while not es.stop() and it < itmax: #iteration loop
            ...
            i = numpy.argmin(f)
            if f[i] < bestSoFar and \
                self.prob.gt.res[i*self.prob.numSamples()] is not None:
                bestSoFar = f[i]
                self.graph.loadValues(
                    self.prob.gt.res[i*self.prob.numSamples()])
        ...
        if it > 0:
            ...
            self.msgQueue.put("Total failed samples: " + \
                str(self.prob.totalSampleErrors))
        else:
            self.msgQueue.put(
