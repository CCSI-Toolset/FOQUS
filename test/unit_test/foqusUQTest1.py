...
try:
    ...
    while(1):
        ...
        if (HAVE_TEXT == 1):
           textedit.insertPlainText('   Clicking the Add Node button (the plus sign on the left panel)\n')
           textedit.ensureCursorVisible() # Scroll the window to the bottom
           if not go(): break
           time.sleep(TIME_STEP)
        ...
        if (HAVE_TEXT == 1):
           textedit.insertPlainText('   Toggling the node edit button (the pencil on left panel)\n')
           textedit.ensureCursorVisible() # Scroll the window to the bottom
           if not go(): break
           time.sleep(TIME_STEP)
        ...
        if (HAVE_TEXT == 1):
           textedit.insertPlainText('         Adding 3 input variables: x1, x2 and x3\n')
           textedit.ensureCursorVisible() # Scroll the window to the bottom
           if not go(): break
           time.sleep(TIME_STEP)
        ...
        if (HAVE_TEXT == 1):
           ...
           if not go(): break
           time.sleep(TIME_STEP)
        MainWin.nodeDock.pyCode.setPlainText("f['y'] = math.sin(x['x1']) + 7 * math.sin(x['x2']) * math.sin(x['x2']) + 0.1 * x['x3'] * x['x3'] * x['x3'] * x['x3'] * math.sin(x['x1'])\ntime.sleep(0.02)")
        if not go(): break
        time.sleep(TIME_STEP)
        ...
        if (HAVE_TEXT == 1):
           textedit.insertPlainText('   Clicking the Add New button, then select sampling and size\n')
           textedit.ensureCursorVisible() # Scroll the window to the bottom
           if not go(): break
           time.sleep(TIME_STEP)
