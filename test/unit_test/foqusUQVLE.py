...
def rsInference(self=self, MainWin=MainWin, getButton=getButton, timers=timers):
    ...
    if "InferenceDialog" in str(type(w)):
        ...
        ### Select the first 2 inputs as design parameters
        ...
        # MainWin.app.processEvents()
        # time.sleep(TIME_STEP)
        # w.activateDiscrepancy()
        # time.sleep(TIME_STEP)
        # pText = '/g/g0/chtong/FOQUS/FOQUS/examples/UQ/test_suite/Phoenix/VLE/psDiscrepancy'
        # w.discrepancySave_edit.setText(pText)
    ...
    while 1:
        ...
        if HAVE_TEXT == 1:
            ...
            if not go():
                break
            time.sleep(TIME_STEP)
        MainWin.dashFrame.sessionNameEdit.setText("VLE inference")
        if not go():
            break
        time.sleep(TIME_STEP)
