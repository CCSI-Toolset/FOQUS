...
global errorTitle
global errorFile
errorFile = "AutoErrLog_data_analysis.txt"
errorCount = 0
...
def uq_sampling_scheme(MainWin=MainWin, getButton=getButton, timers=timers, go=go):
    ...
    if 'SimSetup' in str(type(w)):
        ...
        w.numSamplesBox.setValue(10000)
...
def uq_analyze_scheme(MainWin=MainWin, getButton=getButton, timers=timers, go=go):
    ...
    if 'AnalysisDialog' in str(type(w)):
        ...
        #### Just Sensitivity
        errorTitle = "UQ Analysis with First-Order Sensitivity Analysis"
        w.dataAnalyze_combo1.setCurrentIndex(2)
        w.dataAnalyze_button.click()
        #### Second Order Sensitivity
        errorTitle = "UQ Analysis with Second-Order Sensitivity Analysis"
        w.dataAnalyze_combo2.setCurrentIndex(1)
        w.dataAnalyze_button.click()
        #### Total Order Sensitivity (10,000 runs needed)
        errorTitle = "UQ Analysis with Total-Order Sensitivity Analysis"
...
MainWin.close()
timerWait('msg_no')
print("Exited Code: UQ Data Analysis")
