...
class surrogateFrame(_surrogateFrame, _surrogateFrameUI):
    ...
    def __init__(self, dat, parent=None):
        ...
        self.dataBrowser.editFiltersButton.clicked.connect(
            self.updateFilters)
        self.toolBox.currentWidget().layout().addWidget(
            self.dataBrowser)
    ...
    def addSamples(self):
        ...
        if n1 - n0 == 1:
            ...
            #get launch button
            lb = self.mainWin.uqSetupFrame.simulationTable.\
                cellWidget(row,col)
            lb.clicked.emit()
    ...
    def applyChanges(self, tool=None):
        ...
        for i in range(self.inputTable.rowCount()):
            ...
            var.setMin(gh.getCellJSON(
                self.inputTable, i, self.inputCols['Min']))
            var.setMax(gh.getCellJSON(
                self.inputTable, i, self.inputCols['Max']))
            if c[0]:
                inputs.append(c[1])
                for item in pg.inputCols:
                    col = self.inputCols[item[0]]
    ...
    def updateStatus(self):
        ...
        if not self.pg.isAlive(): done = True
