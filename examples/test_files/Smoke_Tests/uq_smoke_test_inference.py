...
"""
This python code is supposed to test the inference system.
Currently there's something happening that forces this script to continue to completion
before calling the inference function in charge of plotting this.
...
global errorTitle
global errorFile
errorFile = "AutoErrLog_uq_inference.txt"
errorCount = 0
...
def uq_infer_scheme(MainWin=MainWin, getButton=getButton, timers=timers, go=go):
    ...
    w = MainWin.app.activeWindow()
    global errorTitle
    errorTitle = "Fill Inference Scheme and Launch"
    if "InferenceDialog" in str(type(w)):
        ...
        response_select = w.output_table.cellWidget(1, 2).findText(
            "K Nearest Neighbors"
        )
        w.output_table.cellWidget(1, 2).setCurrentIndex(response_select)
        errNum = errorCount
...
try:  # Catch any exception and stop all timers before finishing up
    while 1:  # Loop and break and break as convenient way to jump to end
        ...
        if not timerWait("uq_analyze_close"):
            break
