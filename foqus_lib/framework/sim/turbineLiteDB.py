...
class turbineLiteDB():
    ...
    def add_new_application(self, applicationName, rc=0):
        ...
        try:
            ...
            if curs.fetchone() is None:
                sqlstr="INSERT INTO Applications VALUES ('{0}', NULL, NULL)"\
    ...
    def add_message(self, msg, jobid, rc=0):
        ...
        try:
            ...
            sqlstr="INSERT INTO Messages VALUES ('{0}', '{1}', '{2}', '{3}')"\
                .format(uid, msg, t, jobid)
            curs.execute(sqlstr)
            self.closeConnection()
        except Exception as e:
            self.closeConnection()
    ...
    def consumer_keepalive(self, uid, rc=0):
        ...
        try:
            t = time.strftime("%m/%d/%Y %I:%M %p",time.gmtime())
            sqlstr="UPDATE JobConsumers SET keepalive='{0}' WHERE Id='{1}'"\
                .format(t, uid)
            curs.execute(sqlstr)
            self.closeConnection()
        except Exception as e:
    ...
    def consumer_status(self, uid, status=None, rc=0):
        ...
        try:
            if status is not None:
                sqlstr="UPDATE JobConsumers SET status='{0}' WHERE Id='{1}'"\
                    .format(status, uid)
                curs.execute(sqlstr)
                self.closeConnection()
            else:
                sqlstr = "SELECT status FROM JobConsumers WHERE Id='{0}'"\
                    .format(uid)
                curs.execute(sqlstr)
                s = curs.fetchone()
                self.closeConnection()
    ...
    def consumer_id(self, pid, rc=0):
        conn, curs = self.getConnection()
        try:
            sqlstr = "SELECT Id FROM JobConsumers WHERE processId='{0}' AND status='up'"\
                .format(pid)
            curs.execute(sqlstr)
            s = curs.fetchone()
            self.closeConnection()
    ...
    def consumer_register(self, rc=0):
        ...
        try:
            uid = str(uuid.uuid4())
            t = time.strftime("%m/%d/%Y %I:%M %p",time.gmtime())
            sqlstr = "INSERT INTO JobConsumers "\
                    "VALUES "\
                    "('{0}', 'localhost', {1}, 'up', '{2}', 'foqus')"\
                    .format(uid, os.getpid(), t)
            curs.execute(sqlstr)
            self.closeConnection()
            return uid
        except Exception as e:
    ...
    def jobConsumerID(self, jid, cid=None, rc=0):
        ...
        try:
            if cid is None:
                ...
                sqlstr = "SELECT ConsumerId FROM Jobs WHERE Id='{0}'"\
                    .format(jid)
                curs.execute(sqlstr)
                row = curs.fetchone()
                self.closeConnection()
                ...
                sqlstr = "UPDATE Jobs SET ConsumerId='{0}' WHERE Id='{1}'"\
                    .format(cid, jid)
                curs.execute(sqlstr)
                self.closeConnection()
            return cid
    ...
    def get_configuration_file(self, simulationId, rc=0):
        ...
        conn, curs = self.getConnection()
        try:
            sqlstr = "SELECT Content FROM SimulationStagedInputs "\
    ...
    def job_prepare(self, jobGuid, jobId, configFile, rc=0):
        ...
        conn, curs = self.getConnection()
        try:
            sqlstr = "SELECT Input FROM Processes WHERE Id='{0}'"\
                .format(jobGuid)
            curs.execute(sqlstr)
            row = curs.fetchone()
            self.closeConnection()
    ...
    def job_change_status(self, jobGuid, status, rc=0):
        ...
        try:
            t = time.strftime("%m/%d/%Y %I:%M %p",time.gmtime())
            sqlstr = "UPDATE Jobs SET State='{0}' WHERE Id='{1}'"\
                .format(status, jobGuid)
            curs.execute(sqlstr)
            if status == 'setup':
                sqlstr = "UPDATE Jobs SET Setup='{0}' WHERE Id='{1}'"\
                    .format(t, jobGuid)
                curs.execute(sqlstr)
            elif status == 'running':
                sqlstr = "UPDATE Jobs SET Running='{0}' WHERE Id='{1}'"\
                    .format(t, jobGuid)
                curs.execute(sqlstr)
            elif status in ['success', 'error', 'terminate', 'cancel']:
                sqlstr = "UPDATE Jobs SET Finished='{0}' WHERE Id='{1}'"\
                    .format(t, jobGuid)
                curs.execute(sqlstr)
            self.closeConnection()
        except Exception as e:
    ...
    def job_save_output(self, jobGuid, workingDir, rc=0):
        ...
        try:
            with open(os.path.join(workingDir, "output.json")) as outfile:
                output = outfile.read()
            sqlstr = "UPDATE Processes SET Output='{0}' WHERE Id='{1}'"\
                .format(output, jobGuid)
            curs.execute(sqlstr)
            self.closeConnection()
        except Exception as e:
            self.closeConnection()
