...
class checkingThread(QtCore.QThread):
    ...
    def run(self):
        ...
        launchButton = self.parent.simulationTable.cellWidget(row, self.parent.launchCol)
        ...
        goagain = False
        while (numTries < 10000000 and (numUnfinished > 0 or runID < maxRuns)) or goagain:
            if runType != Model.GATEWAY:
                ...
                if not gt.isAlive():
                    if gt.errorStat==19:
                        raise Exception(
                            "Exception in graph thread (see log)")
...
##                                        'response surface constructed from the ensemble data.')
        ...
        self.delete_table.verticalScrollBar().valueChanged.connect(self.scrollDeleteTable)
    ...
    def simSelected(self):
        ...
        maxWidth = 2 + self.infoTable.verticalHeader().width() + self.infoTable.columnWidth(0)
...
'''
        ...
        if numError == 0:
            ...
            if numGood == 0:
                ...
                styleMiddle = 'stop: 0 lightgrey, stop: %g lightgrey, stop: %g red, ' % \
                              ((numGood - 0.4)/numTotal, (numGood + 0.4)/numTotal)

            if numGood + numError == numTotal:
                styleMiddle = styleMiddle + 'stop: 1 red'
            else:
                styleMiddle = styleMiddle + 'stop: %g red, stop: %g lightgrey, stop: 1 lightgrey' % \
                              ((numGood + numError - 0.4) / numTotal, (numGood + numError + 0.4) / numTotal )
    ...
    def loadSimulation(self):
        ...
        fileName, selectedFilter = QFileDialog.getOpenFileName(self, "Open Simulation Ensemble",
                                               '' , "Psuade Files (*.dat *.psuade *.filtered);;CSV (Comma delimited) (*.csv);;All files (%s)" % allFiles)
        if len(fileName) == 0:
            self.unfreeze()
            return
    ...
    def updateSimTableRow(self, row):
        ...
        if runCount == data.getNumSamples() or data.getFromFile() == True \
                                            or data.getNumSamplesAdded() > 0: # Samples added from refinement
            editButton.setText('View')
        else:
        ...
        #print 'resize'
        self.resizeColumns()
        minWidth = 2 + self.simulationTable.columnWidth(0) + self.simulationTable.columnWidth(1) + \
                   self.simulationTable.columnWidth(2) + self.simulationTable.columnWidth(3) + \
    ...
    def launchSim(self):
        ...
        if sender.text() == 'Launch':
            ...
            outfile = SampleRefiner.adaptiveSample(fname, y, sim.getOrigNumSamples(), numToAdd)
            if outfile is None:
                return
...
# =========================== START Brenda's stuff =========================
    ...
    def initDelete(self):
        ...
        self.delete_table.setHorizontalHeaderLabels(('Variables',) + inputNames + outputNames)
