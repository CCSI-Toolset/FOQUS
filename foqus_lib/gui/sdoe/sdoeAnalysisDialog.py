...
from foqus_lib.gui.uq.AnalysisInfoDialog import *
from foqus_lib.gui.uq.InferenceDialog import *
from foqus_lib.gui.common.InputPriorTable import InputPriorTable
import foqus_lib.gui.uq.RSCombos
...
class sdoeAnalysisDialog(_sdoeAnalysisDialog, _sdoeAnalysisDialogUI):
    ...
    def __init__(self, idNum, data, parent=None):
        super(sdoeAnalysisDialog, self).__init__(parent)
        self.setupUi(self)
        self.data = data
        self.parent = parent
        ...
        # Num columns
        item = QTableWidgetItem(str(data.getNumColumns()))
        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
        flags = item.flags()
        item.setFlags(flags & mask)
        item.setForeground(Qt.black)
        self.infoTable.setItem(self.numColumnsRow, 0, item)

        # Candidate File
        item = QTableWidgetItem(data.getCandFile())
        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
        flags = item.flags()
        item.setFlags(flags & mask)
        item.setForeground(Qt.black)
        ...
        # History File
        item = QTableWidgetItem(data.HistFile())
        item.setTextAlignment(Qt.AlignRight | Qt.AlignVCenter)
        flags = item.flags()
        item.setFlags(flags & mask)
        ...
        # Output Directory
        item = QTableWidgetItem(data.getOutputDir())
        flags = item.flags()
        item.setFlags(flags & mask)
        item.setForeground(Qt.black)
    ...
    def initWizardAnalysisGroup(self):
        ...
        self.wizardRS_combo2.init(self.data, self.wizardRSLegendre_spin,
                                  self.wizardRSLegendre_static)  # Must be called after legendre spinbox
