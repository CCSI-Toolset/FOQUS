...
class surrogate(threading.Thread):
    ...
    def checkNumVars(self):
        ...
        if self.nInput() < self.minInputs:
            self.msgQueue.put(
                "**Must select at least {0} input variables"\
                .format(self.minInputs))
            err = 1
        if self.nInput() > self.maxInputs:
            self.msgQueue.put(
                "**Must select at most {0} input variables"\
                .format(self.maxInputs))
            err = 1
        if self.nOutput() < self.minOutputs:
            self.msgQueue.put(
                "**Must select at least {0} output variables"\
                .format(self.minOutputs))
            err = 1
        if self.nOutput() > self.maxOutputs:
            self.msgQueue.put(
                "**Must select at most {0} output variables"\
