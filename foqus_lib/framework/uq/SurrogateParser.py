...
class SurrogateParser:
    ...
    def writeBssAnovaDriver(bssanovaData, outfile):
        ...
        outputNames = bssanovaData['outputNames']  # list of strings indicating each output's variable name
        modelNames = bssanovaData['modelNames']    # list of strings indicating path of each output's rdsfile
        rscript = bssanovaData['rscriptPath']      # string indicating path to Rscript executable
        rfile = bssanovaData['rfile']              # R script that gets written
        
        # write driver file
        f = open(outfile, 'w')
        frontmatter = ['#!/usr/bin/python',
                       '###################################################',
        ...
        f.write('\n'.join(backmatter) + '\n')
        f.write('rdsfile = models[index]\n')  # pass in the appropriate model file specific to that output
        f.write("p = subprocess.Popen([r'%s', r'%s', rdsfile, infileName, outfileName], \n" % (rscript, rfile))
    ...
    def writeAcossoDriver(acossoData, outfile):
        ...
        outputNames = acossoData['outputNames']  # list of strings indicating each output's variable name
        modelNames = acossoData['modelNames']    # list of strings indicating path of each output's rdsfile
        rscript = acossoData['rscriptPath']      # string indicating path to Rscript executable
