...
class RSVisualization(UQRSAnalysis):
    ...
    def __init__(self, ensemble, output, inputs, responseSurface, minVal, maxVal,
                 rsOptions = None, userRegressionFile = None):
        super(RSVisualization, self).__init__(ensemble, output, UQAnalysis.RS_VISUALIZATION,
    ...
    def analyze(self):
        ...
        if self.legendreOrder is not None:
            mfile = Visualizer.showRS(fname, self.outputs[0], plotInNames, rsdim,
                                      self.responseSurface, vmin=self.minVal,
                                      vmax=self.maxVal, rsOptions=self.rsOptions,
    ...
    def showResults(self):
        ...
        Visualizer.showRSPlot(self.ensemble, self.outputs[0], x, len(self.inputs), ngrid,
                          self.responseSurface, self.minVal, self.maxVal, mfile)
