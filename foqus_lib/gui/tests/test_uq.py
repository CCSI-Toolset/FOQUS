...
class TestUQ(_HasAttributesSetByFixture):
    ...
    def generate_samples(self, qtbot):
        ...
        with qtbot.waiting_for_modal(handler=_accept_dialog):
            ...
            with qtbot.searching_within(
                SimSetup
            ) as sim_frame, qtbot.taking_screenshots():
                with qtbot.searching_within(
                    group_box="Choose how to generate samples:"
                ):
    ...
    def run_simulation(self, qtbot):
        with qtbot.options(take_snapshot_on_locate=False):
            with qtbot.focusing_on(self.frame.simulationTable):
                qtbot.select_row(0)
                # qtbot.wait(5_000)
                with qtbot.intercepting_modal(), qtbot.wait_signal(
                    self.frame.runsFinishedSignal, timeout=90_000
                ):
