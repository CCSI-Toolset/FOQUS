...
class optim():
	...
	def saveDict(self):
		...
		sd["v"]       = self.v
		#
		for g in self.g:   sd["g"].append(g.saveDict())
		for o in self.obj: sd["obj"].append(o.saveDict())
		#
		return sd
	...
	def check(self, minVars = 2):
		...
		for vname in self.v:
			...
			if self.x[vname].scaling == 'None': return [1, "All optimization variables must be scaled check decision variable scaling"]
			min = self.x[vname].min
			max = self.x[vname].max
			if numpy.min(numpy.array(max) - numpy.array(min)) <= 1e-10:
				return [1, "The maximum must be greater than the minimum for all decision variables"]
		if n < minVars: return [1, "Optimization method requires at least " + str(minVars)]
		# Check Objective expressions
		x = self.x
		f = self.f
		if len(self.obj) < 1:
			return[1, "Must have at lease one objective function.  If a single objective method is used the first objective will be be evaluated, and the rest will be ignored"]
		for o in self.obj:
			try:
				eval(o.pycode)
			except Exception, e:
