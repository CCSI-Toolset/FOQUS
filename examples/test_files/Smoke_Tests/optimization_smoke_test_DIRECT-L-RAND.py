...
global errorTitle
global errorFile
errorFile = "AutoErrLog_optimization_smoke_test_DIRECT-L-RAND.txt"
errorCount = 0
...
try:  # Catch any exception and stop all timers before finishing up
    while 1:  # Loop and break and break as convenient way to jump to end
        ...
        if not go():
            break
        MainWin.dashFrame.setSessionDescription(
            "Rosenbrock Optimization Description Text"
        )
        if not go():
            break
        # Make a flowsheet
        ...
        MainWin.optSetupFrame.fTable.setItem(
            0, 0, QtWidgets.QTableWidgetItem("f['Rosenbrock']['f']")
        )
        ...
        solverOptionName = MainWin.optSetupFrame.solverOptionTable.cellWidget(
            0, 1
        ).findText('"DIRECT-L-RAND"')
        MainWin.optSetupFrame.solverOptionTable.cellWidget(0, 1).setCurrentIndex(
            solverOptionName
        )
