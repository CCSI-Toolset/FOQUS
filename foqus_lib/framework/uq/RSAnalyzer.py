...
class RSAnalyzer:
    ...
    def parsePrior(data, xprior):
        ...
        nXpriorVariable = len([1 for prior in xprior if prior is None or prior['type'] != 'Fixed'])
        if nXpriorVariable != nVariableInputs:
            error = 'RSAnalyzer: In function parsePrior(), "xprior" is expected to be a list of length %d.' % nVariableInputs
            Common.showError(error)
            return None
        else:
            inputLB = SampleData.getInputMins(data)
    ...
    def validateRS(fname, y, rsMethodName, rsOptions=None, 
        ...
        if userMethod: # ... user regression ...
            ...
            if setMARS:
                if rsIndex == ResponseSurfaces.MARSBAG:
                    ...
                    f.write('100\n')   # number of MARS instantiations [range:10-5000, default=100]
                                      ### TO DO: revert back to 100 for deployment
                nSamplesCV = math.floor(nSamples/nCV) # number of samples in each CV group
        ...
        if userMethod: # ... user regression ...
            ...
            if len(mres) == 6:
                testErrors = {'max_unscaled': mres[0],
                              'max_scaled': mres[1],
                              'rms_unscaled': mres[2],
                              'rms_scaled': mres[3],
                              'avg_unscaled': mres[4],
            ...
            if len(mres) == 7:
                trainErrors = {'avg_unscaled': mres[0],
                               'avg_scaled': mres[1],
                               'rms_unscaled': mres[2],
                               'rms_scaled': mres[3],
                               'max_unscaled': mres[4],
    ...
    @staticmethod
    def processRSdata(fnameRS, y, data, rsMethodName, rsOptions=None, xprior=None, indexfile=None): 
        ...
        if rsIndex == ResponseSurfaces.LEGENDRE and rsOptions is not None:
            if isinstance(rsOptions, dict):
                ...
                legendreOrder = rsOptions
            outfile = RSAnalyzer.writeRSdata(fnameRS, y, data, rsMethod=rsMethodName, legendreOrder=legendreOrder,
                                             inputLowerBounds=inputLB, inputUpperBounds=inputUB, inputPDF=dist,
                                             indexfile=ifile)
        else:
            outfile = RSAnalyzer.writeRSdata(fnameRS, y, data, rsMethod=rsMethodName, 
                                             inputLowerBounds=inputLB, inputUpperBounds=inputUB, inputPDF=dist,
                                             indexfile=ifile)
        if outfile is None:
            error = 'RSAnalyzer: In function processRSdata(), %s is not written.' % fnameRS
            Common.showError(error)
            return None
    ...
    @staticmethod
    def performUA(fname, y, rsMethodName, rsOptions=None, userRegressionFile=None, xprior=None):
        ...
        data = RSAnalyzer.processRSdata(fnameRS, y, data, rsMethodName, rsOptions=rsOptions, xprior=xprior)
...
#       f.seek(0)
    ...
    def plotUA(data, y, rsMethodName, sfile, mfile):
        ...
        fmt = '%1.4e'
        moments = {'mean': fmt % np.mean(rsdat0),
        ...
        bheights, bedges = np.histogram(rsdat0, bins=bedges)  # use the same bin edges as before
        ...
        pdfs = []
        for d in rsdat:
            bheights, bedges = np.histogram(d, bins=bedges)  # use the same bin edges as before
            bheights_normed = bheights/float(nSamples)
    ...
    @staticmethod
    def performAEUA(fname, y, rsMethodName, rsOptions=None, userRegressionFile=None, xtable=None):
        ...
        if xtable is None:
            ...
            return RSAnalyzer.performUA(fname, y, rsMethodName, rsOptions=rsOptions, userRegressionFile=userRegressionFile)
        ...
        epistemicInVars = [i+1 for i,e in enumerate(xtable) if e['type'] == 'Epistemic']
        ...
        if nEpistemic == 0:
            ...
            return RSAnalyzer.performUA(fname, y, rsMethodName, rsOptions=rsOptions, userRegressionFile=userRegressionFile)
        # ... get aleatory inputs 
        aleatoryInVars = [i+1 for i,e in enumerate(xtable) if e['type'] == 'Aleatory']
        ...
        data = RSAnalyzer.processRSdata(fnameRS, y, data, rsMethodName, rsOptions=rsOptions, 
                                        xprior=xtable, indexfile=indexfile)
        rsIndex = data.getSampleRSType()
    ...
    @staticmethod
    def performSA(fname, y, cmd, showErrorBars, rsMethodName, rsOptions=None, userRegressionFile=None, xprior=None):
        ...
        if showErrorBars and rsMethodName == 'MARSBAG':
            rsMethodName = 'MARS'
            warn = 'RSAnalyzer: In function performSA(), MARSBAG ensemble treated as MARS ensemble for %s analysis.' % cmd.upper()
            Common.showError(warn)
    ...
    def plotSA(data, y, rsMethodName, cmd_, showErrorBars, mfile):
        ...
        if showErrorBars:
            ...
            dat = Plotter.getdata(mfile, datvar)
            std = None
        figtitle = {'rssobol1': 'First-order',
                    'rssobol2': 'Second-order',
                    'rssoboltsi': 'Total-order'}
        title = {'rssobol1': 'Sobol First Order Indices',
        ...
        outVarName = outVarNames[y-1]
        ftitle = '%s Sensitivity Analysis based on %s Response Surface' % (figtitle[cmd_], rsMethodName.upper())
    ...
    @staticmethod
    def emulate(fnameRS, fnameInputSamples, y,
                  textDialog = None, 
        ...
        f.seek(0)
        # ... invoke psuade
        out, error = Common.invokePsuade(f, textDialog = textDialog, 
        ...
        # read traindata
        dataTrain = LocalExecutionModule.readSampleFromPsuadeFile(fnameTrain)   # rstype/order written to data
        ...
        out, error = Common.invokePsuade(fnameIn, textDialog = textDialog, 
                                          dialogShowSignal = dialogShowSignal,
                                          dialogCloseSignal = dialogCloseSignal,
                                          textInsertSignal = textInsertSignal,
                                          ensureVisibleSignal = ensureVisibleSignal,
        ...
        # check output file
        if os.path.exists(psfile):
            psfile_ = Common.getLocalFileName(RSAnalyzer.dname, fnameInputSamples, '.testdat')
