...
def heatIntegrationCalc(node):
    ...
    hotUTin    = dict( [("HP_Steam", 230), ("MP_Steam", 164)] )             # inlet temperature of hot utility (C)
    coldUTin   = dict( [("Cooling_Water", 20)] )                            # inlet temperature of cold utility (C)
    hotUTout   = dict( [("HP_Steam", 229), ("MP_Steam", 163)] )             # outlet temperature of hot utility (C)
    ...
    node.outVars["HP_Steam.Consumption"].value = Uhot["HP_Steam"]         # high-pressure steam consumption
    node.outVars["MP_Steam.Consumption"].value = Uhot["MP_Steam"]         # medium-pressure steam consumption
    node.outVars["Cooling_Water.Consumption"].value = Ucold["Cooling_Water"]         # cooling water consumption
    if feedIs:
        node.outVars["FH.Heat.Addition"].value = [0, 0, 0, 0, 0]
        for i in range(len(feedSet)):
            node.outVars["FH.Heat.Addition"].value[i] = float((f.readline()).strip())      # heat addition to feed water heater
    else:
