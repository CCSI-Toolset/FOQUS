...
#    Embed the snobfit driver within the optimize routine function
    def optimize(self):
        ...
        self.msgQueue.put("Starting SNOBFIT Optimization at {0}".format(
...
#       Calling the snobfit driver to perform optimization
        request,xbest,fbest=SQSnobFit._snobfit.minimize(self.f, xinit, bounds, budget)
...
#        *********
        ...
        self.msgQueue.put("{0}, Total Elasped Time {1}s, Obj: {2}, xbest: {3}"\
                        .format(
                        self.prob.iterationNumber,
                        math.floor(eltime),
                        self.bestSoFar,xbest_unscaled))
