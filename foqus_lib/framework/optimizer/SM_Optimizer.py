...
if __name__ == '__main__':
    from user_plugins import *  # pylint: disable=import-error
...
class opt(optimization):
    ...
    def __init__(self, dat=None):
        ...
        self.options.add(
            name='Solver options',
            default={'tol': 1e-6, 'max_iter': 40, 'halt_on_ampl_error': 'yes',
                     'linear_solver': 'ma27', 'warm_start_init_point': 'yes'},
            dtype=object,
...
#        Creating Pyomo variables corresponding to non surrogate vars
        ...
        for i, nonsurrin in enumerate(self.nonsurrin_names):
            ...
            self.m.add_component('{0}'.format(nonsurrin),
                                 Var(bounds=(minv, maxv), initialize=initv))
...
#        Obtaining list of FOQUS node input & output variable names
        self.sim_input_vars_original = ['x.' + v for v in
                                        self.surrin_names_original +
                                        self.nonsurrin_names_original]
        self.sim_output_vars_original = ['f.' + v for v
                                         in self.surrout_names_original +
                                         self.nonsurrout_names_original]
        self.sim_input_vars_pyomo = ['self.m.' + str(v) for v in
                                     self.surrin_names_pyomo +
                                     self.nonsurrin_names_pyomo]
        self.sim_output_vars_pyomo = ['self.m.' + str(v) for v in
...
#        function and constraints
        ...
        for k in self.var_map_dict.keys():
            if k in o[:]:
                o = re.sub(r'(?<![a-z]){0}(?![a-z])'.format(k),
                           str(self.var_map_dict[k]), o)
        ...
        if len(constrexprs) != 0:
            ...
            for k in constrexprs:
                ...
                for k in self.var_map_dict.keys():
                    if k in g[:]:
                        g = re.sub(r'(?<![a-z]){0}(?![a-z])'.format(k),
                                   str(self.var_map_dict[k]), g)
...
#       Solving the optimization problem with user provided options
    ...
    #   Implementing multi-start approach
        ...
        if multistart is True:
            ...
            for var in decvars:
                initvals.append([var.value, var.lb, var.ub,
                                 (var.lb + var.ub)/2])
            ...
            solver_termination_condition = np.zeros(len(initvals_prod),
                                                    dtype=object)
...
#        Linear scaling of decision variables
        for i, d in enumerate(self.prob.v):
            ...
            dvar_scaled.append(10*(dvar.value - dvar_min_unscaled) /
    ...
    # *** Running Simulations for each sample point
            ...
            for s in range(len(latin_hypercube_samples)):
                ...
                for d in range(len(surrin_pyomo)):
                    surrin_min_bound = \
                        simin.get(self.surrin_names_original[d]).min
                    surrin_max_bound = \
                        simin.get(self.surrin_names_original[d]).max
