...
try:
    ...
    while(1):
        ...
        if not go(): break
        MainWin.nodeDock.inputVarTable.item(0, 4).setText("-10")
        if not go(): break
        MainWin.nodeDock.inputVarTable.item(0, 5).setText("10")
        if not go(): break
        MainWin.nodeDock.inputVarTable.item(1, 4).setText("-10")
        if not go(): break
        MainWin.nodeDock.inputVarTable.item(1, 5).setText("10")
        if not go(): break
        MainWin.nodeDock.inputVarTable.item(0, 1).setText("5")
        if not go(): break
        MainWin.nodeDock.inputVarTable.item(1, 1).setText("2")
        if not go(): break
